...
import auto_uv.preflight as preflight
...
def _patch_probe_dependencies(monkeypatch, *, voltage_issues=None, architecture=10):
    monkeypatch.setattr(preflight.shutil, "which", lambda command: "/usr/bin/nvidia-smi")
    monkeypatch.setattr(
        preflight,
...
def test_preflight_passes_when_driver_and_capability_probes_are_sane(monkeypatch):
    _patch_probe_dependencies(monkeypatch)

    result = preflight.run_auto_uv_preflight(
...
def test_preflight_rejects_old_driver_even_when_capability_probes_pass(monkeypatch):
    _patch_probe_dependencies(monkeypatch)
...
def test_preflight_rejects_old_gpu_by_failed_probes_not_by_name(monkeypatch):
    ...
    assert "Blackwell GeForce RTX 50-series" in message
    assert "Ada Lovelace GeForce RTX 40-series" in message
    assert preflight.AUTO_UV_SUPPORT_ISSUE_URL in message


def test_preflight_rejects_nvml_voltage_reader_failure(monkeypatch):
    ...
    assert "nvml-voltage-reader" in _issue_ids(result)
    assert "0mV" in result.format_for_user()


def test_preflight_rejects_nvapi_setter_failure(monkeypatch):
    _patch_probe_dependencies(monkeypatch)
...
def test_preflight_rejects_nvapi_reader_factory_exception(monkeypatch):
    _patch_probe_dependencies(monkeypatch)
...
def test_preflight_rejects_too_few_vf_points(monkeypatch):
    _patch_probe_dependencies(monkeypatch)
...
def test_driver_version_accepts_major_only_580():
    assert preflight._validate_driver("580") == []
    assert preflight._validate_driver("580.01") == []
    assert preflight._validate_driver("579.99")
