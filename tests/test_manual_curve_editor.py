...
from manual_uv_curve_editor import (
    ManualCurveEdit,
...
def test_manual_add_curve_point_snaps_to_vf_bin_and_clock_step() -> None:
    ...
    added = manual_add_curve_point_edit(
        edit,
        requested_voltage_mv=876,
        requested_clock_mhz=2532,
    )

    assert added.edit_kind == "add-point"
    assert added.selected_voltage_mv == 880
    assert added.selected_clock_mhz == 2535
    assert added.control_voltage_mvs == (880,)
    assert _target_at(added.plan, 880) == 2535


def test_manual_add_curve_point_selects_existing_control_without_reediting() -> None:
    edit = ManualCurveEdit(
        plan=_plan(),
        anchor_voltage_mv=900,
        anchor_clock_mhz=2550,
        ...
        selected_voltage_mv=900,
        selected_clock_mhz=2550,
        control_voltage_mvs=(880,),
    )

    selected = manual_add_curve_point_edit(
        edit,
        requested_voltage_mv=876,
        requested_clock_mhz=2400,
    )

    assert selected.edit_kind == "select"
    assert selected.selected_voltage_mv == 880
    assert selected.selected_clock_mhz == 2520
    assert selected.control_voltage_mvs == (880,)
    assert _target_at(selected.plan, 880) == 2520
