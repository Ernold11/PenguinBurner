...
## 2026-07-10 — RuntimeSpec simplification and live verification

- Replaced runtime argv replay with an immutable typed `RuntimeSpec`; Python
  resolves profiles/config once and Rust validates/applies it synchronously.
- Split current-session recovery (`/run`) from explicit boot persistence
  (`/var/lib`), and made explicit stop clear session recovery.
- Deleted the Rust TOML parser, saved-profile/tier loader, saved-fan loader,
  runtime argv parser, and raw NVML backend. Documented NVML now uses
  `nvml-wrapper`; raw FFI remains only for undocumented NVAPI curve access.
- Added GPU UUID binding, initial memory/VF/power readbacks, previous-profile
  rollback, stock fallback, startup-replay fallback, and wedged-engine refusal.
- Reused the Rust stock-reset pipeline for Auto-UV through one semantic RPC;
  removed the duplicate Python reset workflow and dead Python adaptive runtime
  state machine (Python now supplies only resolved policy values).
- Replaced custom OS plumbing with `nix`, `signal-hook`, `shared_child`,
  `procfs`, `tempfile`, and `sd-notify`.
- Rust is now 14,899 physical lines in `src`, or 15,978 including the integration
  test file (13,354 code lines by `scc`). Compared with the pre-refactor branch
  snapshot, that removes 1,649 physical Rust lines, 1,194 Rust code lines, and
  312 measured complexity points.
- Removed the superseded Python runtime log formatter, overlay-state writer,
  latency sample/frame-generation helpers, in-process marker bridge, and other
  test-only migration helpers. The live overlay remains: Rust publishes state,
  Python reads it for the GUI preview, and the Steam wrapper launches the
  detached per-game marker drainer. Production Python is now 45,752 physical
  lines / 39,977 code lines, down 1,460 / 1,103 from the same snapshot. Combined
  Python + Rust is down 3,329 physical lines, 2,417 code lines, and 609 measured
  complexity points. Including untracked source/test files, the complete git
  working tree is net-negative by 2,193 physical lines, or 2,947 lines when the
  generated 754-line Flatpak Cargo source manifest is excluded.
- Current verification: Rust unit/integration suites, the full Python suite, and
  touched-production-file Pyright are green. Live typed reads, semantic stock
  reset, wrong-UUID rollback, restart recovery, and all 127 V/F readbacks passed
  on the RTX 5080.
- Fixed the live supported-graphics-clock truncation by pinning
  `nvml-wrapper 0.12.0`, whose explicit insufficient-size retry works. Version
  0.12.1 accepts that status in its shared count helper before its public retry
  can run. The final daemon now gets the driver's exact 389 steps from 180 to
  3090 MHz: 2698 MHz floors to 2692 MHz and 2724 floors to 2722 MHz, with no raw
  documented-NVML workaround or local reconstruction code.
- Installed and activated the Rust 0.6.5 daemon on this host. A complete
  Balanced Auto-UV scan passed its 225-second Q2RTX plus 75-second CUDA final
  verification and saved a verified 850 mV / 2664 MHz / 319 W profile. Applying
  that saved profile through `RuntimeSpec` matched all 127 V/F readbacks, and
  the final cleanup restored the GPU to stock (360 W, zero memory offset, zero
  V/F offsets) with the daemon idle. Existing profile files remained unchanged.
...
- Autostart simplification: base64-env path dropped. Typed current-session and
  opt-in boot state are stored separately; the legacy `last-runtime.json` is
...
- **Tests: 96 unit + 12 integration (A1) green; 2 ignored = read-only hardware
  ...
  - overlay-state.txt key order, escaping, and byte layout against the captured
    pre-port Python fixture.
...
7. **Engine journal lines go raw to stdout** (parity with Python `runtime_debug.log`
   ...
   prefix, preserving the concise tag-and-section grammar.
...
3. **In-app marker bridge not ported.** The real marker drainer is the detached
   per-game wrapper process. The unused in-process compatibility class was
   removed after the Rust receiver took ownership of runtime telemetry.
