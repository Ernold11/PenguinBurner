...
4. Watchdog replaces "hope the child is fine" — and adds hang detection today's
   design lacks.
5. End state (milestone A complete): `runtime/gpu_control/`,
   `runtime/fan_control/`, the `runtime_service.py` run path, and the
   `daemon_api.py` server are deleted from Python (~6k LoC), replaced by this
   crate. `daemon_client.py` stays.
...
- **A0** scaffold + toolchain gates — **A1** supervisor/protocol golden-tested
  ...
  hardware verify.
- **B** (only after A verified on hardware): low-level GPU *write* RPCs on the
  ...
  Rust; then de-root the scan child if the persistence/paths spec confirms
  nothing else needs privilege. Python engine deletion completes milestone B.
