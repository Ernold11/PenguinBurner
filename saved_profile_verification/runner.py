"""Foreground runner for verifying saved undervolt profiles."""
...
from .applied_vf_plan_check import (
...
from .target_voltage_abort_rule import profile_verification_voltage_abort_callback
...
class ProfileVerificationDependencies:
    ...
    flattened_clock_ceiling_controller_factory: Callable = (
    ...
    verify_profile_vf_plan: Callable = verify_profile_vf_plan
    ...
    mark_auto_uv_profile_verification_failed: Callable = (
        mark_auto_uv_profile_verification_failed
    )
    mark_auto_uv_profile_verified: Callable = mark_auto_uv_profile_verified
    run_stability_test: Callable = run_stability_test
    build_stability_config: Callable = build_stability_config
    build_cuda_stability_config: Callable = build_cuda_stability_config
    ...
    run_cuda_stability_test: Callable = run_cuda_stability_test
    attach_stdout_progress: Callable = attach_stdout_progress
    print_q2rtx_stability_result: Callable = print_q2rtx_stability_result
    stability_workload_selection: Callable = stability_workload_selection
    stability_workload_label: Callable = stability_workload_label
    stability_workload_split_label: Callable = stability_workload_split_label
    apply_verify_auto_uv_profile: Callable | None = None
    apply_verify_afterburner_profile: Callable | None = None
    run_profile_verification_baseline_probe: Callable | None = None
    log: Callable[[str], None] = runtime_log


def _dependencies(
    dependencies: ProfileVerificationDependencies | None,
) -> ProfileVerificationDependencies:
    return dependencies or ProfileVerificationDependencies()
    ...
    *,
    ...
    try:
        ...
        if prefer_afterburner_curve:
            if deps.apply_verify_afterburner_profile is not None:
                label, flatten_target, verify_plan = (
                    deps.apply_verify_afterburner_profile(
                        vf_curve_reader,
            ...
            if deps.apply_verify_auto_uv_profile is not None:
                label, flatten_target, verify_plan = deps.apply_verify_auto_uv_profile(
                    vf_curve_reader,
                    selector,
                    gpu_policy_controller,
                )
        ...
        include_q2rtx, include_cuda = deps.stability_workload_selection(args)
        workload_label = deps.stability_workload_label(
            include_q2rtx=include_q2rtx,
            include_cuda=include_cuda,
        )
        split_label = deps.stability_workload_split_label(
            duration_s,
            include_q2rtx=include_q2rtx,
            include_cuda=include_cuda,
        )
        ...
        stability_config = (
            ...
            if include_q2rtx
            else deps.build_cuda_stability_config(
                args,
                gpu_index=gpu_index,
                config_path=config_path,
            )
        ...
        try:
            result = (
                ...
                if include_q2rtx
                else deps.run_cuda_stability_test(stability_config)
            )
        ...
        if not result.success:
            if (
                ...
                try:
                    failed_path = deps.mark_auto_uv_profile_verification_failed(
                        selector,
                        failure={
        ...
        if not prefer_afterburner_curve and profile_needs_verify_baseline(selector):
            ...
            try:
                ...
                if deps.run_profile_verification_baseline_probe is not None:
                    base_metrics = deps.run_profile_verification_baseline_probe(
                        args,
                        gpu_index=gpu_index,
                        config_path=config_path,
                        base_plan=base_plan,
        ...
        if not prefer_afterburner_curve:
            verified_path = deps.mark_auto_uv_profile_verified(
                selector,
                verification={
                    "workload": workload_label,
    ...
    *,
    ...
    try:
        deps.log(
            ...
            f"{deps.stability_workload_split_label(duration_s, include_q2rtx=include_q2rtx, include_cuda=include_cuda)}."
        )
...
def apply_verify_afterburner_profile(
    ...
    *,
    ...
    source = deps.resolve_afterburner_vf_source(
    ...
    if gpu_policy_controller is not None:
        try:
            profile_settings = deps.load_afterburner_profile_settings(
                profile_path=source["profile_path"],
                section=source["section"],
            )
            translated_gpu_policy = deps.translate_afterburner_gpu_policy(
                profile_settings,
            ...
            deps.apply_translated_gpu_policy(gpu_policy_controller, translated_gpu_policy)
        except Exception as exc:
            translated_gpu_policy = None
            deps.log(f"Skipping Afterburner GPU policy during verification: {exc}")
    vf_apply_result = deps.apply_afterburner_curve_to_reader(
        vf_curve_reader,
        profile_path=source["profile_path"],
        section=source["section"],
