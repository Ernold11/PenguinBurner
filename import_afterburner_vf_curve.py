...
from import_afterburner_fan_curve import DEFAULT_CONFIG_PATH, load_config, write_config
...
def load_afterburner_runtime_options(config_path):
    ...
    return {
        ...
        "afterburner_device_profile": str(gpu.get("afterburner_device_profile", "")).strip(),
...
def translated_linux_profile_is_current(payload, *, curve_sha256, preserve_vanilla_below_mv=None):
    if int(payload.get("schema_version", 0)) != TRANSLATED_LINUX_PROFILE_SCHEMA_VERSION:
        return False
    if str(payload.get("curve_sha256", "")).strip().lower() != str(curve_sha256).lower():
        return False
...
def apply_offsets_payload(reader, payload):
    ...
    for point in reader.editable_core_points():
        control.vf_points[point["index"]].prog.freq_offset_khz = offsets.get(point["index"], 0) * 1000
...
def write_translated_linux_profile(
    ...
    *,
    ...
    payload = {
        ...
        "source_payload_path": str(Path(source_payload_path)) if source_payload_path else "",
        "points": [
            {
    ...
    if materialization is not None:
        ...
        if materialization.get("flatten_override") is not None:
            payload["afterburner_materialization"]["flatten_override"] = materialization.get(
                "flatten_override"
            )
...
def build_plan(reader, afterburner_points, *, preserve_vanilla_below_mv=None):
    ...
    for point in reader.editable_core_points():
        ...
        preserve_vanilla = (
            preserve_vanilla_below_mv is not None
            and int(voltage_mv) <= int(preserve_vanilla_below_mv)
        )
        target_mhz = int(base_mhz) if preserve_vanilla else int(round(ab_point["frequency_mhz"]))
...
def apply_plan(reader, plan):
    control = reader.get_control_struct()
    for item in plan:
        control.vf_points[item["index"]].prog.freq_offset_khz = item["new_offset_mhz"] * 1000
    reader.set_control_struct(control)
...
def resolve_afterburner_curve_translation(
    ...
    if translated_profile.exists():
        ...
        if translated_linux_profile_is_current(
            ...
            plan, missing_indices, voltage_mismatches = build_plan_from_offsets_payload(reader, payload)
            translation_mode = str(payload.get("translation_mode", "translated-linux-profile"))
            ...
            if isinstance(cached_materialization, dict):
                materialization = {
                    "mode": str(cached_materialization.get("mode", materialization.get("mode", ""))),
    ...
    if not translation_origin:
        ...
        if materialization["mode"] == "adjusted-anchor-cap":
            ...
            translation_mode = (
                "unsafe-voltage-bin-map" if analysis["nonzero_third_value_count"] else "voltage-bin-map"
            )
    ...
    changed_points = [
        item for item in plan if int(item["current_offset_mhz"]) != int(item["new_offset_mhz"])
    ]
...
def import_afterburner_export_into_managed_root(config_path, runtime_options, *, gpu_index=0):
...
def _persist_afterburner_runtime_state(
    ...
    *,
    ...
    if runtime_options is not None:
        ...
        if power_limit_override_w is not None:
...
def ensure_afterburner_root_configured(config_path, runtime_options, *, gpu_index=0, interactive=True):
    configured_root = str(runtime_options.get("afterburner_root", "")).strip()
    if configured_root:
    ...
    while True:
        ...
        try:
            managed_root = sync_afterburner_export_tree(afterburner_root, managed_afterburner_root())
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help="Optional manual power-limit cap in watts; defaults to the translated Afterburner target",
    )
    parser.add_argument(
    ...
    if args.afterburner_dir.strip():
        runtime_options["afterburner_root"] = str(resolve_afterburner_root(args.afterburner_dir))
    ...
    try:
        if args.restore_file:
            ...
            print(f"Restored offsets for {restored} editable VF points from {args.restore_file}")
            reader.refresh_points()
            return 0
        ...
        translated_gpu_policy = translate_afterburner_gpu_policy(
            profile_settings,
            power_limits=policy_controller.query_power_limits(),
            power_limit_cap_w=runtime_options["power_limit_override_w"],
        )
        ...
        flatten_target = derive_afterburner_dynamic_lock(result["materialization"]["points"])
        print(
        ...
        print(f"Translated GPU policy: {describe_translated_gpu_policy(result['gpu_policy'])}")
        print(
            "Flatten target: "
            f"{describe_afterburner_dynamic_lock(flatten_target)}"
        )
        print(
            "Flatten validation: "
        ...
        print(f"Applied GPU policy: {describe_translated_gpu_policy(result['gpu_policy'])}")
        print(
