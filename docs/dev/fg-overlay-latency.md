...
- The Steam launch line is just the wrapper:
  ...
  `PENGUIN_BURNER` wrapper (`overlay/launcher.py`) sets all the
...
- `overlay/launcher.py` — `PENGUIN_BURNER` wrapper; computes all
...
- `runtime_gpu_control/overlay_state_publisher.py` /
  `overlay/state.py` — `latency_ms` field through to the overlay.
