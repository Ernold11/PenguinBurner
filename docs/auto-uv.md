...
- It can spend a bounded curve-target bump budget when a lower-voltage candidate only misses the loaded-clock floor, or when an FPS/W wall may be improved by a small clock bump.
...
- FPS/W stopping also waits until the curve-target bump budget is spent or disabled. The loaded core-clock floor remains a safety guardrail; Auto-UV does not force the scan down to that floor just to stop on marginal FPS/W gains.
...
Auto-UV may retry the same voltage with a small curve-target bump. The same
budget can also be used at an FPS/W wall if a clock bump improves
temperature-normalized efficiency. The total bump budget is
`--auto-uv-max-clock-drop-pct * --auto-uv-clock-bump-budget-ratio`. The ratio
defaults to `0.5` and is clamped to `0.0..1.0`, so the bump budget cannot exceed
...
against the remaining budget. If the machine crashes during a bump, the next
scan remembers the budget used before that failed bump and caps future bumping
