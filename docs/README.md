# PenguinBurner Documentation

Start at the [project README](../README.md) for the overview and the LACT
comparison.

## For users

- **[Install guide](install.md)** — pip, Flatpak, Fedora COPR, Arch AUR,
  Ubuntu PPA.
- **[Flatpak guide](flatpak.md)** — self-hosted Flatpak install, wrappers,
  update, uninstall, and direct launch commands.
- **[Feature guides](features/README.md)** — automatic tuning, adaptive
  undervolting, the overlay, profiles, curve editors, the silent fan curve.
- **[Latency and frame-generation FPS](features/latency-fg.md)** — how the
  overlay's LAT, base FPS, output FPS, and FG fields are measured.
- **[FurMark power checks](furmark.md)** — local 4K Vulkan before/after notes
  for interpreting runtime profile power draw.
- **[Troubleshooting & FAQ](features/troubleshooting.md)**

## Reference

- **[releases/](releases/)** — older release notes.
