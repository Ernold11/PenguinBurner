...
| File | Change |
|------|--------|
| `native/latency_layer/src/penguinburner_latency_layer.cpp` | per-frame emission, `gpu_render_us`, injection removal, uses emit planner |
...
| `latency_telemetry/receiver.py` | `gpu-render-p95` metric; removed dead `gpu-submit-proxy` / timestamp fields |
| `tests/test_latency_telemetry.py` | `gpu_render_us` tests; removed `gpu-submit-proxy` test |
| `docs/re9-latency-experiment-findings.md` | update noting injection removal + the two read-only fixes; link to debug guide |
...
- Python suite: full run green (412 passed, 17 pre-existing skips);
  `tests/test_latency_telemetry.py` 12 passed.
