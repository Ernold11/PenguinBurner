...
class SteamPanel:
    ...
    def _sync_default_mode_label(self) -> None:
        ...
        default_index = self.mode_combo.findData(GAME_MODE_DEFAULT)
        if default_index >= 0:
            self.mode_combo.setItemText(default_index, self._default_mode_label)
        adaptive_index = self.mode_combo.findData(GAME_MODE_ADAPTIVE)
        if adaptive_index >= 0:
            item = self.mode_combo.model().item(adaptive_index)
            if item is not None:
                item.setEnabled(bool(self.adaptive_available()))
        if not self.adaptive_available():
